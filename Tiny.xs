...
SV*
__get_smaps_summary_mmap(char* filename)
PPCODE:
    struct smaps_sizes sizes;
    memset(&sizes, 0, sizeof sizes);
    HV* hash = newHV();
    ...
    if (fd == -1)
        croak("Couldn't open(%s): [%d] %s", filename, errno, strerror(errno));

    /* To obtain file size */
    if (fstat(fd, &sb) == -1)
        croak("Couldn't stat(%d): [%d] %s", fd, errno, strerror(errno));
    
    void *addr = mmap(NULL, sb.st_size, PROT_READ,
                      0, fd, 0);
    if (addr == MAP_FAILED)
        croak("Couldn't mmap(%d): [%d] %s", fd, errno, strerror(errno));
